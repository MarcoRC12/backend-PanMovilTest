...
class TipoPago extends Controller{
        ...
        foreach($registro as $key=>$value){
            if(array_key_exists('Authorization',$headers) && !empty($headers['Authorization'])){
                if($request->getHeader('Authorization')=='Authorization: Basic '.base64_encode($value['cliente_id'].':'.$value['llave_secreta'])){
                    ...
                    $TipoPago = $model->where('tp_estado',1)->findAll();
                    if(!empty($TipoPago)){
                        $data = array(
                            "Status"=>200,
    ...
    public function show($id){
        ...
        foreach($registro as $key=>$value){
            if(array_key_exists('Authorization',$headers) && !empty($headers['Authorization'])){
                if($request->getHeader('Authorization')=='Authorization: Basic '.base64_encode($value['cliente_id'].':'.$value['llave_secreta'])){
                    $model = new TipoPagoModel();
                    $TipoPago = $model->where('tp_estado',1)->find($id);
                    if(!empty($TipoPago)){
                        $data = array(  
    ...
    public function create(){
        ...
        foreach($registro as $key=>$value){
            if(array_key_exists('Authorization',$headers) && !empty($headers['Authorization'])){
                if($request->getHeader('Authorization')=='Authorization: Basic '.base64_encode($value['TipoPago_id'].':'.$value['llave_secreta'])){
                        $datos = array(
